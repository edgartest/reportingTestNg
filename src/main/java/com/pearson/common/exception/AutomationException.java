...
public class AutomationException extends Exception {
    ...
    /**
     * Will attempt to include the versionId of app into the exception message
     * to enable us to extract that information directly from Jenkins reports
    ...
    private static String generateExceptionMsg(WebDriver driver, String message) {
        ...
        if(url != null && !url.isEmpty()) {
            msg = msg.concat(CommonConstants.NEW_LINE.getValue()).concat(url);
        }
        
        Utils utils = Utils.getInstance(driver);
        if(utils != null) {
            //String versionId = utils.getVersionId();
        	String versionId = utils.getVersionId();
            
            if(versionId != null && !versionId.isEmpty()) {
                msg = msg.concat(CommonConstants.NEW_LINE.getValue()).concat("[[versionId:").concat(versionId).concat("]]");
            }
        }
